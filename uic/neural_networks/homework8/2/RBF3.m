...
plot(c(1,n_c+1:2*n_c),c(2,n_c+1:2*n_c),'ks','MarkerFaceColor','blue')
drawnow
...
w=normrnd(0,1/sqrt(20),1,2*n_c+1); %initial weights
for i=1:n_in
    for j=1:2*n_c
        y(j,i)=f(x(:,i),c(:,j));
    end
...
while flag==1
    ...
    errors(epoch)=0;
    % loop misclassifications errors
    for i=1:n_in
        g=sgn(w*[1;y(:,i)]);
        if d(i)~=g
...
%     if epoch>1
%         if errors(epoch)==10 && flag_eta==1
%             eta=5;
%             flag_eta=0;
%         end
%         if errors(epoch)>errors(epoch-1) && eta>1 && errors(epoch)>10
%             eta=eta*0.999;
%         elseif errors(epoch)>errors(epoch-1) && eta>0.5 && errors(epoch)<10
%             eta=eta*0.999;
%         elseif errors(epoch)>errors(epoch-1) && errors(epoch)<5
%              eta=0.5;
%         end
